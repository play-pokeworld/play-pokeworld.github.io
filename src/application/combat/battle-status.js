...
async function onEnemyFaint(){
 ...
 // Phase 30: shared K.O. counter for the hatchery — each K.O. feeds
 // incubation and daycare according to each slot's mode (single function).
 ...
 const leagueTrainers = (typeof getLeagueTrainersForRegion === 'function') ? getLeagueTrainersForRegion(battle.leagueRegion || 'kanto') : [];
 ...
 addBattleLog(`<span class="pw-green">${tr('league_stage_victory', {trainer:leagueTrainers[battle.leagueStage - 1].name})}</span>`);
 ...
 // Phase 19: official team for the stage (Champion Blue variant resolved)
 const leagueStarterId = (typeof getPlayerStarterSpecies === 'function') ? getPlayerStarterSpecies(battle.leagueRegion || 'kanto') : null;
 const officialStage = (typeof getOfficialLeagueTeam === 'function') ? getOfficialLeagueTeam(battle.leagueRegion || 'kanto', battle.leagueStage, leagueStarterId) : [];
 battle.champTeam = officialStage.length ? officialStage : trainer.team.map(([id, lv]) => createPoke(id, lv, false));
...
if (typeof tickStatusDurations !== 'undefined') { if (typeof window !== 'undefined') window.tickStatusDurations = tickStatusDurations; if (typeof globalThis !== 'undefined') globalThis.tickStatusDurations = tickStatusDurations; }
if (typeof applyEndOfTurnStatus !== 'undefined') { if (typeof window !== 'undefined') window.applyEndOfTurnStatus = applyEndOfTurnStatus; if (typeof globalThis !== 'undefined') globalThis.applyEndOfTurnStatus = applyEndOfTurnStatus; }
// FIX (2026-08, crash on first K.O.): onEnemyFaint MUST be exposed — the
// ECS-driven battle loop (src/application/battle-loop.js, domain tick)
// calls it via `typeof onEnemyFaint === 'function'`; without exposure, K.O.
// resolution never started (battle.paused stayed true => frozen battle).
if (typeof onEnemyFaint !== 'undefined') { if (typeof window !== 'undefined') window.onEnemyFaint = onEnemyFaint; if (typeof globalThis !== 'undefined') globalThis.onEnemyFaint = onEnemyFaint; }
