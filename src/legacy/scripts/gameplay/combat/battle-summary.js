function renderBattleSummary(){
  ...
  if (!battle.isChamp) {
    ...
    if (catches.length || itemKeys.length) {
      ...
      for (const id in agg) {
        const a = agg[id];
        html += `<div data-style="display:flex;align-items:center;gap:10px;background:var(--dark3);border-radius:6px;padding:8px;border:1px solid ${a.shinyCount ? 'var(--light2)' : 'var(--light1)'};">
