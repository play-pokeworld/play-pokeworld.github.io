...
class ItemInfoPanel extends InfoPanel {
  _buildContent() {
    ...
    if (powerDisplay) {
      this._sections.push({
        title: (typeof t==='function' && t('power_label')) || (lang === 'en' ? 'Power' : 'Puissance'),
    ...
    const _il = function(key, fr, en){ var v = (typeof t === 'function') ? t(key) : null; return (v && v !== key) ? v : (lang === 'en' ? en : fr); };
    const typeLabels = {
      stone: _il('evo_stone', 'Pierre Évolutive', 'Evolution Stone'),
      treasure: _il('cat_treasure', 'Trésor', 'Treasure'),
      fossil: _il('cat_fossil', 'Fossile', 'Fossil'),
      evolution: _il('cat_evolution_item', 'Objet d\'Évolution', 'Evolution Item'),
      held: _il('cat_held_item', 'Objet Tenu', 'Held Item'),
      key: _il('cat_key_item', 'Objet Clé', 'Key Item'),
      ct: _il('cat_tm', 'CT', 'TM'),
      cs: _il('cat_hm', 'CS', 'HM'),
      candy: _il('cat_candy', 'Bonbon', 'Candy'),
      memory: 'Memory', z_crystal: 'Z-Crystal',
      keystone: 'Keystone', stone: 'Stone',
    };
    const typeLabel = typeLabels[itm.type] || _il('cat_item', 'Objet', 'Item');
    ...
    if (itm.typeBoost) {
      detailsHtml += this._renderRow(
        lang === 'en' ? 'Type Boosted' : (typeof t==='function'?t('boosted_type'):'Boosted Type'),
        this._typeBadge(itm.typeBoost)
      );
    }
    if (itm.resistType) {
      detailsHtml += this._renderRow(
        lang === 'en' ? 'Resists' : (typeof t==='function'?t('resistance'):'Resistance'),
        `${this._typeBadge(itm.resistType)} ${itm.resistPercent || 30}%`
      );
    }
    if (itm.revive) {
      detailsHtml += this._renderRow(
        lang === 'en' ? 'Revives into' : (typeof t==='function'?t('revives'):'Revives'),
        `#${itm.revive}`
      );
    }
    if (itm.evolution) {
      detailsHtml += this._renderRow(
        lang === 'en' ? 'Evolution' : 'Évolution',
        '✓ ' + (lang === 'en' ? 'Evolves certain Pokemon' : (typeof t==='function'?t('evolves_some_pokemon'):'Evolves certain Pokemon'))
      );
    }
  ...
  _getSource() {
    ...
    if (!db) return '';
    if (lang === 'en') return '📍 Found in: ' + (db.source || db.shop || 'various locations');
...
class MoveInfoPanel extends InfoPanel {
  _buildContent() {
    ...
    const catLabel = mv.category === 'physical' ? (this._lang === 'en' ? 'Physical' : 'Physique')
      : mv.category === 'special' ? (this._lang === 'en' ? 'Special' : (typeof t==='function'?t('cat_special'):'Special'))
      : (this._lang === 'en' ? 'Status' : 'Statut');
    ...
    if (mv.recharge) html += this._renderRow(this._t('recharge') || 'Recharge', this._lang === 'en' ? 'Needs recharge' : 'Doit recharger');
    ...
    if (mv.eff) {
      ...
      const effEN = { burn: 'Burn', para: 'Paralysis', poison: 'Poison', sleep: 'Sleep', freeze: 'Freeze', confuse: 'Confusion', flinch: 'Flinch', slow: 'Slow' };
      const effName = this._lang === 'en' ? (effEN[mv.eff] || mv.eff) : (effNames[mv.eff] || mv.eff);
    ...
    // Description from localization
      var desc = mv.desc || this._t('moves.' + this.key + '.desc') || '';
  // Replace weather/terrain/status terms with colored badges
  desc = typeof replaceWeatherTerms === 'function' ? replaceWeatherTerms(desc) : desc;
