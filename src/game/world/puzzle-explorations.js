// Explorations secrètes — v4.1
// Immersion, pas de jargon UI, séquences sans feedback avant confirmation,
// boutons mélangés, réponses FR, Regi braille + rituel, sanctuaire = équipe réelle.
...
const PUZZLE_EXPLORATIONS = [
  // KANTO
  _seq({
    id: 'cerulean_sigil_a', region: 'kanto', loc: 'ceruleancave',
    name: 'Murmures sous la roche', nameEn: 'Whispers under stone',
    difficulty: 2, icon: '✦',
    summary: 'Quatre steles veillent dans le noir. Une seule procession apaise la grotte.',
  ...
  _seq({
    ...
    clue: 'Quand la nuit se lève la premiere, le reste du monde suit a rebours jusqu à la pierre.',
  ...
  _seq({
    ...
    difficulty: 2, icon: '🌊',
    summary: 'Trois vannes de bronze. La mer n’obéit qu a une mélodie.',
  ...
  }),
  {
    ...
    type: 'code',
    clue: 'Rebus gravé : un flocon au-dessus d’une vague. Les anciens avaient un mot pour ce qui figé l’eau l’hiver.',
  ...
  }),
  {
    ...
    difficulty: 2, icon: '🌙',
    summary: 'Quelle lumière guide les Évoli de ces grottes ?',
    summaryEn: 'Which light guides the Eevee of these caves?',
    type: 'code',
  ...
  _seq({
    ...
    difficulty: 3, icon: '🔤',
    summary: 'Après le jour, la marée ramene les flots. Le chemin se replie sur lui-meme.',
    summaryEn: 'After day, the tide returns. The path folds onto itself.',
    clue: 'Après le voyage du soleil, la marée ramene les flots sur leurs pas — sans s’attarder au point le plus sombre.',
  ...
  _seq({
    ...
    clue: 'Le drame du feu s’écrit dans l’ordre ou il consume.',
  ...
  },
  {
    ...
    difficulty: 2, icon: '🧊',
    summary: 'Un serment est grave dans la paroi bleue.',
  ...
  _seq({
    ...
    difficulty: 3, icon: '🧊',
    summary: 'Quatre dalles glissantes. Une chorégraphie evite la chute.',
    summaryEn: 'Four slick tiles. One choreography avoids the fall.',
    clue: 'Sur le verglas, le corps avance, hésite d’un cote puis de l autre, et reprend appui.',
  ...
  // HOENN
  {
    ...
    // Message braille encodé en ANGLAIS (SEA) — langue universelle du jeu ;
    // les réponses FR et EN sont acceptées quelle que soit la langue.
    brailleCells: ['⠎', '⠑', '⠁'],
    solution: 'SEA',
  ...
  },
  {
    ...
    // Message braille encodé en ANGLAIS (DOOR) ; réponses FR/EN acceptées.
    brailleCells: ['⠙', '⠕', '⠕', '⠗'],
    solution: 'DOOR',
...

  {
    ...
    difficulty: 3, icon: '📜',
    summary: 'Le panneau du fond ne s éveille qu’en presence de deux gardiens des mers.',
  ...
  },
  {
    ...
    // Message braille encodé en ANGLAIS (ICE) ; réponses FR/EN acceptées.
    brailleCells: ['⠊', '⠉', '⠑'],
    solution: 'ICE',
  ...
  },
  {
    ...
    type: 'braille',
    // Message braille encodé en ANGLAIS (STEEL) ; réponses FR/EN acceptées.
  ...
  }),
  {
    ...
    difficulty: 2, icon: '⚙️',
    summary: 'Le tombeau demande le matériau sacre.',
  ...
  _seq({
    ...
    difficulty: 3, icon: '🌊',
    summary: 'Quatre veines de pression. Une seule séquence evite l effondrement.',
...
function renderPuzzleListPanel(locId) {
  ...
  if (!body) return;
  if (!list.length) {
    body.innerHTML = '<div class="pw-empty-state-md">' + (en ? 'Nothing unusual here…' : 'Rien d insolite ici…') + '</div>';
    return;
  }
  let html = '<div class="pw-puzzle-list-hint">' + (en
    ? 'Some places keep quiet secrets. Look closely.'
    : 'Certains lieux gardent des secrets silencieux. Observez bien.') + '</div>';
  for (const p of list) {
    const ever = isPuzzleEverCompleted(p.id);
    const locked = !puzzleRequirementsMet(p);
    const status = locked ? (en ? 'Sealed' : 'Scelle')
      : ever ? (en ? 'Remembered' : 'Déjà percé')
      : (en ? 'Intact' : 'Intact');
    const col = locked ? 'var(--light1)' : ever ? 'var(--green)' : 'var(--yellow,#ffd54f)';
    html += '<div class="pw-puzzle-card"><div class="pw-row" style="justify-content:space-between;align-items:center;gap:10px;"><div>'
      + '<div class="pw-puzzle-card-title">' + (p.icon || '✦') + ' ' + _esc(getPuzzleDisplayName(p))
      + (ever ? ' <span style="color:var(--green)">✓</span>' : '') + '</div>'
      + '<div class="pw-puzzle-card-sum">' + _esc(getPuzzleSummary(p)) + '</div>'
      + '<div class="pw-text-sm" style="margin-top:4px;color:' + col + ';font-weight:700;">' + status + '</div></div>'
      + (locked
        ? '<button class="hbtn disabled" style="opacity:0.55;cursor:not-allowed;">🔒</button>'
        : '<button class="hbtn" data-action="legacy-call" data-call="openPuzzleExploration" data-call-args="\'' + p.id + '\'">'
          + (ever ? (en ? 'Return' : 'Revenir') : (en ? 'Approach' : "S’approcher")) + '</button>')
      + '</div></div>';
  }
  html += '<div class="pw-btn-center" style="margin-top:14px;"><button class="hbtn" data-action="legacy-call" data-call="closeFullscreenPanel" data-call-args="">'
    + (en ? 'Close' : 'Fermer') + '</button></div>';
  body.innerHTML = html;
}
...
function openPuzzleExploration(puzzleId) {
  ...
  let bodyHtml = '';
  if (ever) {
    bodyHtml += '<div class="pw-text-sm" style="color:var(--green);margin-bottom:8px;">'
      + (en ? '✓ You have been here before.' : '✓ Vous êtes déjà passe par ici.') + '</div>';
  }
  bodyHtml += '<div class="pw-puzzle-summary">' + _esc(getPuzzleSummary(puzzle)) + '</div>';
  if (clue) {
    bodyHtml += '<div class="pw-puzzle-inscribe"><div class="pw-puzzle-inscribe-label">'
      + (en ? 'Found here' : 'Sur place') + '</div>' + _esc(clue) + '</div>';
  }

  if (puzzle.type === 'sequence') {
    const opts = _shuffle((puzzle.options || []).map((o) => ({
      key: o.key, label: en ? (o.en || o.fr) : (o.fr || o.en),
    })));
    const buttons = opts.map((o) =>
      '<button type="button" class="hbtn puzzle-seq-btn" data-seq-key="' + _esc(o.key) + '">' + _esc(o.label) + '</button>'
    ).join('');
    bodyHtml += '<div id="puzzle-seq-progress" class="pw-puzzle-progress"></div>'
      + '<div class="pw-puzzle-seq-btns">' + buttons + '</div>'
      + '<input type="hidden" id="puzzle-answer-input" value="" />';
  } else if (puzzle.type === 'wait') {
    bodyHtml += '<div class="pw-text-sm pw-light1" style="margin-bottom:8px;">' + (en ? 'The threshold listens.' : 'Le seuil écoute.') + '</div>'
      
      + '<label class="pw-text-sm" for="puzzle-answer-input">' + (en ? 'Your action' : 'Votre action') + '</label>'
      + '<input id="puzzle-answer-input" class="pw-puzzle-input" type="text" maxlength="48" autocomplete="off" spellcheck="false" placeholder="…" />';
  } else if (puzzle.type === 'party') {
    bodyHtml += '<div class="pw-text-sm pw-light1" style="margin-bottom:8px;">' + (en ? 'Stand before the mural with the right companions.' : 'Présentez-vous devant la murale avec les bons compagnons.') + '</div>'
      + '<input type="hidden" id="puzzle-answer-input" value="PARTY" />';
  } else {
    if (puzzle.brailleCells && puzzle.brailleCells.length) {
      bodyHtml += '<div class="pw-puzzle-braille">' + puzzle.brailleCells.join(' ') + '</div>';
    }
    bodyHtml += '<label class="pw-text-sm" for="puzzle-answer-input">' + (en ? 'Your reading' : 'Votre lecture') + '</label>'
      + '<input id="puzzle-answer-input" class="pw-puzzle-input" type="text" maxlength="48" autocomplete="off" spellcheck="false" placeholder="…" />';
  }

  const shell = '<div class="pw-puzzle-shell">'
    + '<div class="modal-title"><div>' + (puzzle.icon || '✦') + ' ' + _esc(getPuzzleDisplayName(puzzle)) + '</div>'
    + '<span class="modal-close" data-action="legacy-call" data-call="closePuzzleExploration" data-call-args="">✕</span></div>'
    + '<div class="pw-puzzle-body">' + bodyHtml + '</div>'
    + '<div class="pw-puzzle-actions">'
    + '<button class="hbtn pw-btn-cancel" data-action="legacy-call" data-call="closePuzzleExploration" data-call-args="">'
    + (en ? 'Leave' : "S’éloigner") + '</button>'
    + '<button class="hbtn" data-action="legacy-call" data-call="submitPuzzleAnswer" data-call-args="\'' + puzzle.id + '\'">'
    + (en ? 'Confirm' : 'Confirmer') + '</button></div></div>';
  ...
  if (typeof _pwSetHtmlSafe === 'function') _pwSetHtmlSafe(inner, shell);
  else if (typeof pwSetHtml === 'function') pwSetHtml(inner, shell);
  else inner.innerHTML = shell;
  ...
  if (puzzle.type === 'wait') {
    const fill = modal.querySelector('#puzzle-wait-fill');
    const lab = modal.querySelector('#puzzle-wait-label');
    const sec = puzzle.waitSeconds || 60;
    const tick = () => {
      const sess = window._puzzleSession;
      if (!sess || sess.id !== puzzle.id || !modal.classList.contains('open')) return;
      const elapsed = Date.now() - (sess.waitStarted || Date.now());
      const pct = Math.min(100, (elapsed / (sec * 1000)) * 100);
      if (fill) fill.style.width = pct + '%';
      if (lab) {
        if (sess.waitTouched) lab.textContent = en ? 'The stillness was broken.' : "L’immobilité a ete rompue.";
        else if (pct >= 100) lab.textContent = en ? 'The place accepts your patience.' : 'Le lieu accepte votre patience.';
        else lab.textContent = '… ' + Math.floor(elapsed / 1000) + 's';
      }
...
function closePuzzleExploration() {
  ...
  if (inner) {
    if (typeof _pwSetHtmlSafe === 'function') _pwSetHtmlSafe(inner, '');
...
function submitPuzzleAnswer(puzzleId) {
  ...
  if (puzzle.type === 'sequence') {
    ...
    const disturbed = !!(sess && sess.waitTouched);
    ok = waited;
  } else if (puzzle.type === 'party') {
  ...
  if (!ok) {
    ...
    try {
      ...
      if (inp) {
        if (inp.type === 'text' || inp.type === 'search') { inp.value = ''; setTimeout(function(){ try { inp.focus(); } catch (e) {} }, 0); }
...
if (typeof window !== 'undefined') {
  window.PUZZLE_EXPLORATIONS = PUZZLE_EXPLORATIONS;
  window.ensurePuzzleState = ensurePuzzleState;
  window.getPuzzleById = getPuzzleById;
  window.isPuzzleCompleted = isPuzzleCompleted;
  window.isPuzzleEverCompleted = isPuzzleEverCompleted;
  window.isPuzzleSolvedThisRun = isPuzzleSolvedThisRun;
  window.resetPuzzleRun = resetPuzzleRun;
  window.puzzleRequirementsMet = puzzleRequirementsMet;
  window.getPuzzlesForLocation = getPuzzlesForLocation;
  window.countCompletedPuzzles = countCompletedPuzzles;
  window.openPuzzleListForLocation = openPuzzleListForLocation;
  window.renderPuzzleListPanel = renderPuzzleListPanel;
  window.openPuzzleExploration = openPuzzleExploration;
  window.closePuzzleExploration = closePuzzleExploration;
  window.submitPuzzleAnswer = submitPuzzleAnswer;
  window.completePuzzleExploration = completePuzzleExploration;
}
