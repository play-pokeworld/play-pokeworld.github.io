...
var _mineMgmtLastPage = null; // suivi de page pour la conservation du scroll
function openMineManagementMenu(page='upgrades'){
 ...
 // Anti « retour en haut » (passes 15+16) : squelette persistant comme la
 // pension et l'entraînement (conteneur scrollable jamais recréé).
 ...
 let shell = inner.querySelector && inner.querySelector('.management-shell.management-mine');
 ...
 if(tabsHost) tabsHost.innerHTML = renderMineManagementTabs(page);
...
function renderMine(el){
 ...
 contentHtml = `<div class="pw-slot-label">${depth}</div>`;
 }
 ...
 _pwSetHtmlSafe(el, `<div class="hatchery-upgrade-row"><button class="hbtn" data-action="legacy-call" data-call="openMineManagementMenu" data-call-args="'upgrades'">${typeof getIcon==='function'?getIcon('settings',14):''} ${t('mine_management_button')}</button></div>
 <div class="loc-title">${t('mine_title')}</div>
 <div class="loc-sub">${t('mine_sub')}</div>

 <div class="pw-actions-row">
 <div class="pw-action-flex">
 ...
 <div class="pw-field-hint">${t('mine_energy_hint')}</div>
 </div>
 ...
 <div class="pw-trainer-head">
 <div>
 <span class="pw-bold">${t('mine_treasures')} <span class="pw-light2">${foundCount} / ${items.length}</span></span>
 <div class="pw-field-text">
