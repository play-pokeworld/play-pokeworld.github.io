...
function mineAutoStep(){
 ...
 if(fee > 0) {
   if(G.money < fee) {
     ...
     notify((typeof t==='function'?t('mine_auto_no_money'):"Auto-mine disabled: not enough money!"), "var(--red)");
     saveGame();
...
function digMineTile(tx, ty, silent=false){
 ...
 if(G.mine.items && (!G.mine.items.length || G.mine.items.every(it => it.collected))){
  if(!silent) notify((typeof t==='function'?t('mine_depleted_new'):'Layer depleted: new mine generated automatically!'), 'var(--blue)');
  generateMineLayer();
 }
 saveGame();
 try{ autoSave(); }catch(e){}
