...
const STAFF_DEFS = [
 {id:'manager_daisy', type:'hatchery', loc:'cerulean', cost:1200000, bonus:'hatchery_speed', perLevel:0.006, maxLevel:100},
 {id:'manager_celadon', type:'hatchery', loc:'celadon', cost:2200000, bonus:'hatchery_queue', perLevel:0.004, maxLevel:100},
 {id:'manager_saffron', type:'hatchery', loc:'saffron', cost:3200000, bonus:'hatchery_speed', perLevel:0.005, maxLevel:100},
 {id:'manager_cinnabar', type:'hatchery', loc:'cinnabar', cost:4200000, bonus:'hatchery_queue', perLevel:0.005, maxLevel:100},
 {id:'manager_goldenrod', type:'hatchery', loc:'goldenrod', cost:5200000, bonus:'hatchery_speed', perLevel:0.004, maxLevel:100},
 {id:'trainer_vermilion', type:'training', loc:'vermilion', cost:1200000, bonus:'training_ease', perLevel:0.005, maxLevel:100},
 {id:'trainer_saffron', type:'training', loc:'saffron', cost:2500000, bonus:'training_mastery', perLevel:0.004, maxLevel:100},
 {id:'trainer_fuchsia', type:'training', loc:'fuchsia', cost:3400000, bonus:'training_ease', perLevel:0.004, maxLevel:100},
 {id:'trainer_indigo', type:'training', loc:'indigo', cost:5000000, bonus:'training_mastery', perLevel:0.005, maxLevel:100},
 {id:'trainer_blackthorn', type:'training', loc:'blackthorn', cost:6500000, bonus:'training_ease', perLevel:0.004, maxLevel:100},
 {id:'miner_pewter', type:'mine', loc:'pewter', cost:1200000, bonus:'mine_efficiency', perLevel:0.005, maxLevel:100},
 {id:'miner_cerulean', type:'mine', loc:'cerulean', cost:2000000, bonus:'mine_efficiency', perLevel:0.004, maxLevel:100},
 {id:'miner_cinnabar', type:'mine', loc:'cinnabar', cost:2600000, bonus:'mine_energy', perLevel:0.004, maxLevel:100},
 {id:'miner_mahogany', type:'mine', loc:'mahogany', cost:4200000, bonus:'mine_efficiency', perLevel:0.004, maxLevel:100},
 {id:'miner_blackthorn', type:'mine', loc:'blackthorn', cost:6000000, bonus:'mine_energy', perLevel:0.005, maxLevel:100}
];
...
function staffXpNeed(id){
 ...
 return Math.floor(15 * Math.pow(1.08, lvl) + lvl * 10);
}
...
function renderStaffList(type){
 ...
 // Sector fee info
 let sectorName = type === 'hatchery' ? (typeof t==='function'?t('sector_hatchery'):'Daycare') : type === 'training' ? (typeof t==='function'?t('sector_training'):'Training') : (typeof t==='function'?t('sector_mine'):'Mine');
 let feeLabel = type === 'hatchery' ? (typeof t==='function'?t('fee_hatchery'):'auto-hatch') : type === 'training' ? (typeof t==='function'?t('fee_training'):'auto-battle') : (typeof t==='function'?t('fee_mine'):'tile mined');
  let baseFeeVal = type === 'hatchery' ? 1000 : type === 'training' ? 500 : 500;
 let disc = getStaffSectorDiscount(type);
 let curFee = Math.max(0, Math.ceil(baseFeeVal * (1 - disc)));
 let combinedLv = getStaffSectorLevelsCombined(type);
 
 let html = `<div class="staff-summary" data-style="margin-bottom:10px;"><b>${t('staff_active')}</b> ${active.length}/${max}</div>`;
 html += `<div class="staff-summary-tip" data-style="font-size:12px;color:var(--light2);margin-bottom:12px;background:rgba(255,255,255,0.05);padding:8px;border-radius:4px;border-left:3px solid var(--accent);">
   💡 ${typeof t==='function'?t('sector_fee_label'):'<b>Fees for'} ${sectorName} :</b> ${curFee}₽ ${typeof t==='function'?t('sector_fee_per'):'per'} ${feeLabel}.<br>
...
if (typeof getStaffSectorDiscount !== 'undefined' && typeof window !== 'undefined') window.getStaffSectorDiscount = getStaffSectorDiscount;
if (typeof getStaffSectorLevelsCombined !== 'undefined' && typeof window !== 'undefined') window.getStaffSectorLevelsCombined = getStaffSectorLevelsCombined;
if (typeof getHatcheryAutomationFee !== 'undefined' && typeof window !== 'undefined') window.getHatcheryAutomationFee = getHatcheryAutomationFee;
if (typeof getHatcheryLevelUpFee !== 'undefined' && typeof window !== 'undefined') window.getHatcheryLevelUpFee = getHatcheryLevelUpFee;
if (typeof getTrainingAutomationFee !== 'undefined' && typeof window !== 'undefined') window.getTrainingAutomationFee = getTrainingAutomationFee;
if (typeof getMineAutomationFee !== 'undefined' && typeof window !== 'undefined') window.getMineAutomationFee = getMineAutomationFee;
