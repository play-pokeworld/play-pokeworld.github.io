function renderBox(el){
 ...
 el.innerHTML= battleLockBanner + filtersHtml + `<div class="pw-empty-state">
 ${t('box_empty')}
 </div>${swap?`<div class="pw-btn-center"><button class="hbtn" data-action="legacy-call" data-call="cancelBoxSwap" data-call-args="">${t('finish_btn')}</button></div>`:''}`;
 return;
 }
 if(!entries.length){
 ...
 <div class="pw-bold pw-text-sm">${isShiny?'<span class="shiny-tag" title="Forme Shiny"></span>':''}${typeof getPokeName==='function'?getPokeName(poke.id):poke.name}</div>
 <div class="pw-text-sm pw-light1">Nv.${poke.level}</div>
