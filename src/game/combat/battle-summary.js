...
function renderBattleSummary(){
  ...
  if (!battle.isChamp) {
    ...
    if (data.captureGroups.length || data.itemEntries.length) {
      ...
      inlineHtml = `<div class="pw-text-sm pw-light1">${t('no_loot_yet') || 'Aucun butin récolté pour le moment.'}</div>`;
    }
    ...
    if (data.damageEntries.length) {
      ...
      html += `<div class="battle-summary-empty">${t('battle_summary_no_damage') || (typeof t==='function'?t('battle_summary_no_damage'):'No damage recorded for this session.')}</div>`;
    }

    html += `<div class="battle-summary-section-title">${t('captured_pokemon_title')}</div>`;
    if (data.captureGroups.length) {
      html += `<div class="pw-col pw-gap-sm">`;
      for (const a of data.captureGroups) {
        html += `<div class="battle-summary-entry ${a.shinyCount ? 'is-shiny' : 'is-normal'}">
          <div class="pw-summary-sprite">${spriteImg(a.id, a.emoji || '', {shiny: a.shinyCount > 0, size: 40})}</div>
          <div class="pw-flex-1">
            <div class="pw-summary-name">${a.shinyCount ? '<span class="pw-shiny-star">★</span>' : ''}${a.name}</div>
            <div class="pw-text-sm pw-light1">${a.dupeCount ? `${t('battle_summary_duplicates') || 'Doublons'}: ${a.dupeCount}` : (a.shiny ? (t('battle_summary_shiny') || 'Shiny') : (t('battle_summary_standard') || 'Standard'))}</div>
          </div>
          <div class="pw-summary-count">&times;${a.count}</div>
        </div>`;
      }
      html += `</div>`;
    } else {
      html += `<div class="battle-summary-empty">${t('battle_summary_no_captures') || (typeof t==='function'?t('battle_summary_no_captures'):'No captures during this session.')}</div>`;
    }

    html += `<div class="battle-summary-section-title">${t('found_items_title')}</div>`;
    if (data.itemEntries.length) {
      html += `<div class="pw-row-wrap pw-gap-sm">`;
      for (const [key, qty] of data.itemEntries) {
        html += `<div class="pw-card-bordered pw-row pw-gap-sm battle-summary-entry is-normal">
          ${itemSpriteHtml(key, 32)}
          <div>
            <div class="pw-bold pw-text-sm pw-light2">${getItemName(key)}</div>
            <div class="pw-text-sm pw-light1">&times;${qty}</div>
          </div>
        </div>`;
      }
      html += `</div>`;
    } else {
      html += `<div class="battle-summary-empty">${t('battle_summary_no_items') || (typeof t==='function'?t('battle_summary_no_items'):'No items found during this session.')}</div>`;
    }
  } else {
    html = `<div class="pw-text-sm pw-light1">${t('champ_no_loot') || "Les combats d'arène n'accordent pas de capture de Pokémon sauvage."}</div>`;
    inlineHtml = '';
  }
