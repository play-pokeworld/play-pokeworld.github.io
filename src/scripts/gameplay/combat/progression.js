...
function getEvolutionMethodsHtml(id){
 ...
 if(methods.length === 0){
 return `<div style="background:var(--dark2);border-radius:6px;padding:8px;margin-bottom:10px;font-size:13px;color:var(--light1);border:1px solid #333;">
 <b>${t("m.progression.4")}</b> ${t("m.progression.3")}
 </div>`;
 }

 return `<div style="background:var(--dark2);border-radius:6px;padding:8px;margin-bottom:10px;font-size:13px;color:var(--light2);border:1px solid #333;">
 <div style="font-weight:bold;color:var(--light2);margin-bottom:6px;"> ${t("m.progression.2")}</div>
 ${methods.join('')}
 </div>`;
}
...
function evolve(p,targetId){
 ...
 notify(` ${p.name} déclenche une évolution : ${evo.name} apparaît dans la boîte PC !`,'var(--accent)');
