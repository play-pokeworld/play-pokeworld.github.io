...
function handleStatusBeforeMove(poke, side){
 ...
 addBattleLog(`${poke.name} se réveille !`);
 } else {
 addBattleLog(`${poke.name} est endormi... (${poke.statusTurns} tour${poke.statusTurns>1?'s':''})`);
 updateBattleUI();return false;
 }
 }
 if(s==='freeze'){
 // 20% chance to thaw each turn, OR thaw automatically when duration expires
 if(poke.statusTurns && poke.statusTurns > 0){
 poke.statusTurns--;
 if(poke.statusTurns <= 0){ poke.status=null; poke.statusTurns=0; addBattleLog(`${poke.name} se décongèle !`); return true; }
 }
 if(chance(20)){poke.status=null;poke.statusTurns=0;addBattleLog(`${poke.name} se décongèle !`);}
...
function executeAttack(attacker, defender, moveId, side){
 ...
 // Stat move — status effects ONLY apply to defender
 if(mv.cat==='stat'){
 ...
 defender.statusTurns=2; // Durée paralysie: 2 tours (Pokéchill)
 ...
 defender.statusTurns=3; // Durée poison: 3 tours (Pokéchill)
 addBattleLog(lang==='en' ? `${defender.name} was poisoned!` : `${defender.name} est empoisonné !`);
 } else {
 addBattleLog(lang==='en' ? `${attacker.name}'s attack missed!` : `${attacker.name} rate son coup !`);
 }
 ...
 defender.statusTurns=0; // badpoison = persistant (Toxik), dégâts croissants
 addBattleLog(lang==='en' ? `${defender.name} was badly poisoned!` : `${defender.name} est gravement empoisonné !`);
 } else {
 addBattleLog(lang==='en' ? `${attacker.name}'s attack missed!` : `${attacker.name} rate son coup !`);
 }
 } else if(mv.eff==='burn'){
 ...
 defender.statusTurns=3; // Durée brûlure: 3 tours (Pokéchill)
 addBattleLog(lang==='en' ? `${defender.name} was burned!` : `${defender.name} est brûlé !`);
 } else {
 addBattleLog(lang==='en' ? `${attacker.name}'s attack missed!` : `${attacker.name} rate son coup !`);
 }
 ...
 defender.statusTurns=rand(1,3); // Durée sommeil: 1-3 tours
 addBattleLog(lang==='en' ? `${defender.name} fell asleep!` : `${defender.name} s'est endormi !`);
 } else {
 addBattleLog(lang==='en' ? `${attacker.name}'s attack missed!` : `${attacker.name} rate son coup !`);
 }
 ...
 defender.statusTurns=3; // Durée gel: 3 tours max (Pokéchill)
 addBattleLog(lang==='en' ? `${defender.name} was frozen!` : `${defender.name} est gelé !`);
 } else {
 addBattleLog(lang==='en' ? `${attacker.name}'s attack missed!` : `${attacker.name} rate son coup !`);
 }
 } else if(mv.eff==='slow'){
 ...
 attacker.statusTurns = 2; // Durée paralysie: 2 tours
 ...
 attacker.statusTurns = 3; // Durée poison: 3 tours
 ...
 if(lang === 'en'){
 addBattleLog(`${defender.name} lost ${dmg} HP!${effMsg?' <span style="color:var(--light2)">'+effMsg+'</span>':''}`);
 if(critMult>1) addBattleLog(`<span style="color:var(--orange)">Critical hit!</span>`);
 } else {
 addBattleLog(`${defender.name} perd ${dmg} PV !${effMsg?' <span style="color:var(--light2)">'+effMsg+'</span>':''}`);
 ...
 // IMPORTANT: only applies to the defender (target), NEVER the attacker
 ...
 defender.status='burn';defender.statusTurns=3;addBattleLog(lang==='en' ? `${defender.name} was burned!` : `${defender.name} est brûlé !`);
 } else if(mv.eff==='para'){
 if(!(defender.type1==='Electric'||defender.type2==='Electric'||defender.type1==='Ground'||defender.type2==='Ground')){
 defender.status='para';defender.statusTurns=2;addBattleLog(lang==='en' ? `${defender.name} was paralyzed!` : `${defender.name} est paralysé !`);
 }
 } else if(mv.eff==='poison'){
 if(defender.type1!=='Poison'&&defender.type2!=='Poison'&&defender.type1!=='Steel'&&defender.type2!=='Steel'){
 defender.status='poison';defender.statusTurns=3;addBattleLog(lang==='en' ? `${defender.name} was poisoned!` : `${defender.name} est empoisonné !`);
 }
 } else if(mv.eff==='freeze'&&defender.type1!=='Ice'&&defender.type2!=='Ice'){
 defender.status='freeze';defender.statusTurns=3;addBattleLog(lang==='en' ? `${defender.name} was frozen!` : `${defender.name} est gelé !`);
 } else if(mv.eff==='sleep'){
 defender.status='sleep';defender.statusTurns=rand(1,3);addBattleLog(lang==='en' ? `${defender.name} fell asleep!` : `${defender.name} s'est endormi !`);
 }
 }
 }
