...
function exportSave(){
 ...
 exportBox.innerHTML = `
 <div style="font-size: 13px;color:var(--light2);font-weight:bold;margin-bottom:6px;"> ${t("m.save.4")}</div>
 <textarea id="export-textarea"style="width:100%;height:100px;background:#0c0a09;color:#fff;font-family:monospace;font-size: 13px;padding:6px;border:1px solid #333;border-radius:4px;resize:none;"readonly onclick="this.select()">${data}</textarea>
 ...
 <a class="hbtn"href="data:application/json;charset=utf-8,${encodeURIComponent(data)}"download="pokeworld_save.json"style="flex:1;padding:8px;background:var(--blue);color:#fff;font-weight:bold;text-align:center;text-decoration:none;border-radius:6px;display:flex;align-items:center;justify-content:center;">⬇ ${t("m.save.2")}</a>
 </div>
 `;
 const ta = document.getElementById('export-textarea');
