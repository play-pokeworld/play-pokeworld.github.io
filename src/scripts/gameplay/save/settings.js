...
function debugGiveMoney(){
 G.money += 50000;
 updateHeader();
 notify(t("n.50_000_ajoutés"), 'var(--light2)');
}
function debugGiveCandies(){
 addToInventory('rarecandy', 10);
 notify(t("n.10_super_bonbons_ajoutés"), 'var(--accent)');
 showTab('inventory');
}
...
function toggleBattleSpeedX10(){
  ...
  notify(debugX10Enabled ? '⚡ Vitesse x10 activée' : 'Vitesse x10 désactivée', debugX10Enabled ? 'var(--green)' : 'var(--light1)');
}
