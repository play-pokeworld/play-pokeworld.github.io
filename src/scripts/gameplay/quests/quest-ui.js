...
function renderStoryWindow(){
  ...
  const qCard = (inst, cat, def, numLabel)=>{
    ...
    let body;
    if(def.type==='badge'){
      body = `<div style="font-size:11px;color:${done?'var(--green)':'var(--gold)'};font-weight:bold;margin:2px 0 6px;">${done?(t("m.quest_ui.25")):(t("m.quest_ui.24"))}</div>`;
    } else if(def.type==='talk'){
      body = `<div style="font-size:11px;color:${done?'var(--green)':'var(--gold)'};font-weight:bold;margin:2px 0 6px;">${done?(t("m.quest_ui.23")):(t("m.quest_ui.22"))}</div>`;
    } else {
      body = `<div style="display:flex;justify-content:space-between;font-size:11px;color:var(--dim);"><span>${t("m.quest_ui.21")}</span><span style="color:${done?'var(--green)':'var(--gold)'};font-weight:bold;">${done?'✅':prog+' / '+tgt}</span></div>\n        <div style="height:6px;background:#221e1c;border-radius:3px;overflow:hidden;margin:4px 0 6px;"><div style="width:${pct}%;background:${done?'var(--green)':'var(--gold)'};height:100%;transition:width .3s;"></div></div>`;
    }
  ...
  if(mainDef){
    ...
    html += `<div style="font-weight:bold;color:var(--gold);font-size:12px;margin:2px 0 6px;border-bottom:1px solid #3a322c;padding-bottom:3px;">📖 ${t("m.quest_ui.19")} (${regionName} — ${step}/${total})</div>`;
    html += qCard(mainInst, 'main', mainDef, step+'.');
  }
  if(sideActive.length){
    html += `<div style="font-weight:bold;color:var(--blue);font-size:12px;margin:10px 0 6px;border-bottom:1px solid #3a322c;padding-bottom:3px;">🗣️ ${t("m.quest_ui.18")} (${regionName})</div>`;
    for(const inst of sideActive){ const def=SIDE_QUESTS[inst.qid]; if(def) html+=qCard(inst,'side',def,''); }
  }
  if(repsActive.length){
    html += `<div style="font-weight:bold;color:var(--purple);font-size:12px;margin:10px 0 6px;border-bottom:1px solid #3a322c;padding-bottom:3px;">🔁 ${t("m.quest_ui.17")} (${tr("m.repeatable_limit", {n: (G.maxRepeatables||3)})})</div>`;
    for(const inst of repsActive){ const def=inst.def; if(def) html+=qCard(inst,'repeatable',def,''); }
  }
  html += `<div style="margin-top:8px;">\n    <button class="hbtn" style="width:100%;background:var(--purple);color:#fff;font-size:11px;font-weight:bold;" onclick="openRepeatableMenu()">🔁 ${t("m.quest_ui.16")}</button>\n  </div>`;
...
function openNpc(locId, idx){
  ...
  if(npc.quest && SIDE_QUESTS[npc.quest]){
    ...
    if(active){
      html += `<div style="color:var(--gold);font-size:12px;margin:6px 0;">📌 ${t("m.quest_ui.15")} ${ttl}</div>`;
    } else if(done){
      html += `<div style="color:var(--green);font-size:12px;margin:6px 0;">✅ ${t("m.quest_ui.14")} ${ttl}</div>`;
    } else {
  ...
  if(npc.board){
    html += `<button class="hbtn" style="background:var(--purple);color:#fff;font-weight:bold;width:100%;margin-top:6px;" onclick="openRepeatableMenu()">🔁 ${t("m.quest_ui.12")}</button>`;
  }
  html += `<div style="text-align:right;margin-top:10px;"><button class="hbtn" onclick="closeQuestModal()">✕ ${t("m.quest_ui.11")}</button></div>`;
  if(talkedMain){
    html += `<div style="color:var(--gold);font-size:12px;margin:6px 0;">📖 ${t("m.quest_ui.10")}</div>`;
  }
  const tEl=document.getElementById('quest-title'); if(tEl) tEl.textContent = '🗣️ ' + npcName;
...
function acceptSideQuest(sid){
  ...
  if(G.activeQuests.some(i=>i.qid===sid && i.cat==='side')){ notify(t("n.quête_déjà_active"),'var(--gold)'); return; }
...
function renderRepeatableBoard(){
  const lang=G.lang||'fr';
  let html = `<div style="font-size:12px;color:#ccc;margin-bottom:8px;">${t("m.quest_ui.7")}</div>`;
  html += _repeatableRoll.map((tpl,i)=>{
    ...
    return `<div style="background:rgba(156,39,176,0.1);border:1px solid var(--purple);padding:8px;border-radius:6px;margin-bottom:8px;">
      ...
      <div style="font-size:11px;color:#ccc;margin:4px 0;">${dsc}</div>
      <div style="font-size:11px;color:var(--gold);margin-bottom:6px;">🎁 ${rew}</div>
      ${active?`<div style="color:var(--green);font-size:11px;">✅ ${t("m.quest_ui.6")}</div>`:`<button class="hbtn" style="background:var(--purple);color:#fff;font-weight:bold;width:100%;" onclick="acceptRepeatable(${i})">✅ ${t("m.quest_ui.5")}</button>`}
    </div>`;
  }).join('');
...
function acceptRepeatable(i){
  ...
  if((G.repeatables||[]).length >= (G.maxRepeatables||3)){
    const lang = G.lang||'fr';
    notify(tr("m.quest_ui.2", {p0:G.maxRepeatables||3}),'var(--purple)');
    return;
  }
