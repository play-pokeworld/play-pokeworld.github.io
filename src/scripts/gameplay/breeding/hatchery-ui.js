...
function renderHatcheryWindow(){
 ...
 if(!slot){
 html += `<button class="hbtn"style="width:100%;padding:10px;background:rgba(30,136,229,0.18);border:1px dashed var(--blue);color:#fff;font-weight:bold;font-size: 13px;"onclick="openUnifiedSelectorModal('hatchery')">➕ ${isEn?`Slot #${i+1}: Place Pokémon`:`Slot #${i+1} : Déposer un Pokémon`}</button>`;
 } else {
 const p = slot.poke;
 ...
 <div style="width: 40px; height: 40px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.05);border-radius:6px;cursor:pointer;"onclick="openUnifiedSelectorModal('box_view')">
 ${isFossil ? itemIcon(slot.fossilKey,40) : spriteImg(displayId, displayEmoji, {size:60, shiny:displayShiny})}
 </div>
 <div style="flex:1;min-width:0;">
 <div style="font-weight:bold;font-size:13px;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${iconEmoji} ${displayName} <span style="font-size: 13px;color:var(--light1)">Slot #${i+1}</span></div>
 <div style="font-size: 13px;color:var(--light1);margin:4px 0;">${done ? (isEn?'Ready!':'Prêt !') : (isEn?`Incubating: ${steps} / ${req} KOs`:`Incubation : ${steps} / ${req} KO`)}</div>
 <div style="height:6px;background:#221e1c;border-radius:3px;overflow:hidden;border:1px solid #111;">
 <div style="width:${pct}%;background:${done?'var(--green)':(isFossil?'var(--light2)':'var(--blue)')};height:100%;transition:0.3s;"></div>
 </div>
 </div>
 ${done ? `<button class="hbtn"style="padding:6px 10px;background:var(--green);color:#fff;font-weight:bold;font-size:13px;"onclick="hatchEgg(${i})"> ${isEn?'Hatch!':'Éclore'}</button>` : ''}
 </div>`;
 }
 }
 if(maxSlots < 4){
 const upgradeCost = maxSlots * 15000;
 html += `<button class="hbtn"style="width:100%;margin-top:4px;padding:7px;background:#2a2012;border:1px solid var(--light2);color:var(--light2);font-size:13px;font-weight:bold;"onclick="upgradeHatcherySlots(${upgradeCost})">⬆ ${isEn?`Upgrade Daycare: +1 Egg Slot (${upgradeCost.toLocaleString()}₽)`:`Améliorer Pension : +1 Slot (${upgradeCost.toLocaleString()}₽)`}</button>`;
 }
 html += `</div>`;
 el.innerHTML = html;
}
...
function renderFossilLab(el){
 ...
 if(!fossils.length){
 html += `<div style="text-align:center;padding:30px;color:var(--light1);background:var(--dark3);border-radius:8px">
 <div style="font-size: 15px;margin-bottom:8px">⛏</div>
 <b>${isEn ? 'No fossils yet' : 'Aucun fossile'}</b><br>
 <span style="font-size: 13px">${isEn ? 'Dig in the Underground Mine to find Helix, Dome & Old Amber!' : 'Creusez dans la Mine Souterraine pour trouver des fossiles Nautile, Dôme et Vieil Ambre !'}</span>
 <div style="margin-top:12px"><button class="hbtn"style="background:var(--blue);color:#fff"onclick="showTab('mine')">⛏ ${isEn ? 'Go to Mine' : 'Aller à la Mine'}</button></div>
 </div>`;
 el.innerHTML = html;
 return;
 }
 ...
 html += `<div style="background:var(--dark3);border:1px solid #4a3c2e;border-radius:10px;padding:12px;display:flex;flex-direction:column;gap:8px">
 <div style="display:flex;align-items:center;gap:10px">
 <div style="font-size: 15px">${itemIcon(f.key,36)}</div>
 <div style="flex:1">
 <div style="font-weight:bold">${getItemName(f.key)}</div>
 <div style="font-size:13px;color:var(--light1)">${isEn ? 'Qty' : 'Qté'}: <b style="color:var(--light2)">${f.qty}</b></div>
 </div>
 </div>
 ...
 <div style="font-size:13px;color:var(--light1)">${isEn ? 'Revives into' : 'Ranime en'}</div>
 ${owned ? `<div style="font-size:13px;color:var(--green)"> ${isEn ? 'Owned' : 'Possédé'}</div>` : ''}
 </div>
 </div>
 ...
 html += `</div>`;
 html += `<div style="margin-top:14px;background:rgba(255,193,7,0.08);border:1px solid rgba(255,193,7,0.25);padding:10px;border-radius:8px;font-size: 13px;color:var(--light1)">
