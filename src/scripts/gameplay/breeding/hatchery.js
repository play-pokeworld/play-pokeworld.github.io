...
function reviveFossil(fossilKey){
 ...
 if(G.team.length < 6){
 G.team.push(p);
 notify((isEn ? `🧬 ${p.name} revived! Joined your party!` : `🧬 ${p.name} ranimé ! Il rejoint votre équipe !`), isShiny ? 'var(--light2)' : 'var(--green)');
 } else {
 G.collection[pokeId] = p;
 notify((isEn ? `🧬 ${p.name} revived! Sent to PC Box.` : `🧬 ${p.name} ranimé ! Envoyé au PC.`), isShiny ? 'var(--light2)' : 'var(--green)');
 }
