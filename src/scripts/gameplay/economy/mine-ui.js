...
function renderMineWindow(){
 ...
 if(!mineUnlocked()){
 el.innerHTML = `<div style="text-align:center;padding:10px;color:var(--light1);font-size:13px;"> ${lang==='en'?'Mine locked (2 Badges required)':'Mine verrouillée (2 Badges requis)'}</div>`;
 return;
 }
 renderMine(el);
}
...
function renderMine(el){
 ...
 if(depth === 0){
 if(cellItem){
 contentHtml = `<div style="width:100%;height:100%;background:${cellItem.collected?'rgba(76,175,80,0.25)':'rgba(148,136,107,0.25)'};display:flex;align-items:center;justify-content:center;border-radius:3px">${isItemCenter ? itemIcon(cellItem.key, 24) : ''}</div>`;
 }
 } else {
 const hint = (depth === 1 && cellItem) ? `<div style="width:7px;height:7px;border-radius:50%;background:rgba(255,255,255,0.4)"></div>` : '';
 contentHtml = `<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:13px;color:rgba(255,255,255,0.35);font-weight:bold">${hint||depth}</div>`;
 }
 ...
 <div style="font-size:13px;color:var(--light1);margin-bottom:4px;display:flex;justify-content:space-between">
 <span>${t('mine_energy')}</span>
 <span id="mine-energy-val"style="color:var(--light2);font-weight:bold">${energy||0} / ${maxEnergy||100}</span>
 </div>
 <div class="stat-bar"style="height:12px;background:var(--dark2)"><div id="mine-energy-bar"class="stat-fill"style="width:${((energy||0)/(maxEnergy||100))*100}%;background:var(--light2)"></div></div>
 <div style="font-size: 13px;color:var(--light1);margin-top:3px">${t('mine_energy_hint')}</div>
 </div>

 <div style="display:flex;gap:6px">
 <button class="hbtn"style="${tool==='chisel'?'background:var(--light2);color:#000;font-weight:bold':''}"onclick="setMineTool('chisel')">${t('mine_chisel')}</button>
 <button class="hbtn"style="${tool==='hammer'?'background:var(--light2);color:#000;font-weight:bold':''}"onclick="setMineTool('hammer')">${t('mine_hammer')}</button>
 </div>
 </div>

 ${gridHtml}

 <div style="display:flex;justify-content:space-between;align-items:center;background:var(--dark3);padding:10px 14px;border-radius:8px;flex-wrap:wrap;gap:10px">
 <div>
 <span style="font-weight:bold">${t('mine_treasures')} <span style="color:var(--light2)">${foundCount} / ${items.length}</span></span>
 <div style="font-size:13px;color:var(--light1);margin-top:4px">
