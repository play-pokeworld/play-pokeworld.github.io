...
function renderBox(el){
 ...
 if(!entries.length){
 el.innerHTML= battleLockBanner + `<div style="text-align:center;padding:40px;color:var(--light1)">
 ${t('box_empty')}
 ...
 <button class="hbtn"style="padding:4px 8px;font-size:13px;background:var(--blue);color:#fff;font-weight:bold;"onclick="openUnifiedSelectorModal('box_view')">🔍 ${typeof G!=='undefined'&&G&&G.lang==='en'?'Full-Screen PC Box':'Agrandir la Boîte'}</button>
 </div>`;
 ...
 return `
 <div class="box-card"style="cursor:pointer;${isShiny?'border:1px solid var(--light2);background:rgba(148,136,107,0.06);box-shadow:0 0 8px rgba(148,136,107,0.3);':''}"onclick="openBoxPokeModal('${id}')"oncontextmenu="event.preventDefault(); openBoxPokeModal('${id}'); return false;"title="Clic ou Clic Droit pour voir la fiche">
 <div class="ab-icon${isShiny?' shiny-spark is-shiny':''}">${spriteImg(poke.id,poke.emoji,{shiny:isShiny,size:40})}</div>
 <div style="font-weight:bold;font-size: 13px">${isShiny?'<span class="shiny-tag"title="Forme Shiny"></span>':''}${poke.name}</div>
 <div style="font-size: 13px;color:var(--light1)">Nv.${poke.level}</div>
 <div class="box-actions"onclick="event.stopPropagation()">
 <button class="hbtn"style="padding:3px 6px;font-size:13px"onclick="openBoxPokeModal('${id}')"title="Sheet">${t("fiche_btn")}</button>
 ${swap
 ? `<button class="hbtn"style="padding:3px 6px;font-size:13px"onclick="swapBoxWithTeam('${id}')"${battle.active?'disabled title="Combat en cours"':''}>${t("swap_btn")}</button>`
 : `<button class="hbtn"style="padding:3px 6px;font-size:13px"onclick="addBoxedToTeam('${id}')"${G.team.length>=6||battle.active?'disabled':''}>${t("team_btn")}</button>`}
 </div>
 </div>`;
 }).join('')}
 </div>`;
...
function statusLabel(s){
 return {burn:'BRÛ',poison:'☠POI',badpoison:'☠TXQ',para:'PAR',sleep:'SOM',freeze:'🧊GEL',confuse:'💫CON'}[s]||s.toUpperCase();
}
