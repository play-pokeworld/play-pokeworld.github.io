// ===== FR — Quest texts (main / side / repeatable) =====
// quests.<cat>.<id>.{title,desc,rewardDesc}
window.L_fr_quests = {
 "main": {
  "30": {
   "title": "Les Conseils du Prof. Chen",
   "desc": "Au Bourg Palette, parlez au Prof. Chen pour recevoir ses precieux conseils de depart. C'est votre toute premiere quete !",
   "rewardDesc": "300₽ + 5 Baies Oran"
  },
  "0": {
   "title": "Le Début d'une Aventure",
   "desc": "Bienvenue dans PokéWorld ! Rendez-vous sur la Route 1 et remportez 3 combats sauvages pour prouver votre talent.",
   "rewardDesc": "500₽ + 5 Baies Oran"
  },
  "40": {
   "title": "La Forêt de Jadielle",
   "desc": "Explorez la Forêt Jadielle et remportez 5 combats sauvages pour aiguiser votre equipe.",
   "rewardDesc": "600₽ + 5 Baies Sitrus"
  },
  "41": {
   "title": "La Route 22 Sauvage",
   "desc": "La Route 22 regorge de Pokémon. Remportez 6 combats sauvages pour devenir plus fort !",
   "rewardDesc": "700₽ + 5 Baies Ceriz"
  },
  "1": {
   "title": "Le Baptême Roche d'Argenta",
   "desc": "Rendez-vous à Argenta et defiez Pierre dans son Arene Roche. Triomphez pour remporter votre premier Badge !",
   "rewardDesc": "1 000₽ + 1 Fossile Ancien"
  },
  "42": {
   "title": "L'Énigme du Mont Sélénite",
   "desc": "Traversez la Route 3 et remportez 6 combats sauvages pour progresser vers le Mont Sélénite.",
   "rewardDesc": "1 500₽ + 1 Pierre Lune"
  },
  "43": {
   "title": "La Traversée de la Route 4",
   "desc": "La Route 4 mene a Azuria. Remportez 6 combats sauvages pour forger votre equipe.",
   "rewardDesc": "1 200₽ + 5 Baies Prine"
  },
  "2": {
   "title": "L'Expédition du Mont Sélénite",
   "desc": "Explorez le sombre Mont Sélénite. Vainquez 6 Pokémon sauvages pour deterrer une Pierre Lune.",
   "rewardDesc": "1 500₽ + 1 Pierre Lune"
  },
  "3": {
   "title": "Le Pont Pépite d'Azuria",
   "desc": "À Azuria, triomphez d'Ondine (Misty) dans son arène pour obtenir le Badge Cascade !",
   "rewardDesc": "2 500₽ + 1 Pépite d'or"
  },
  "44": {
   "title": "Les Eaux de la Route 5",
   "desc": "La Route 5 relie Azuria a Safrania. Remportez 6 combats sauvages pour vous entrainer.",
   "rewardDesc": "1 500₽ + 5 Baies Sitrus"
  },
  "45": {
   "title": "La Route 6 — Vers Carmin",
   "desc": "Empruntez la Route 6 vers Carmin sur Mer et remportez 6 combats sauvages.",
   "rewardDesc": "1 500₽ + 5 Baies Oran"
  },
  "5": {
   "title": "Le Tonnerre de Carmin sur Mer",
   "desc": "Naviguez jusqu'a Carmin sur Mer et affrontez le Major Bob dans son arene Electrik pour decrocher le Badge Foudre.",
   "rewardDesc": "3 500₽ + 1 Pierre Foudre"
  },
  "4": {
   "title": "Les Secrets de la Cave Taupiqueur",
   "desc": "Traversez la Route 11 et explorez la mystérieuse Cave Taupiqueur. Remportez 5 combats sauvages pour renforcer votre equipe.",
   "rewardDesc": "3 000₽ + 5 Baies Sitrus"
  },
  "46": {
   "title": "L'Écluse de la Route 9 et 10",
   "desc": "Les Routes 9 et 10 menent au nord. Remportez 6 combats sauvages pour progresser.",
   "rewardDesc": "2 000₽ + 5 Baies Prine"
  },
  "47": {
   "title": "Le Tunnel Rocheux",
   "desc": "Francchissez le Tunnel Rocheux vers Lavanville. Remportez 6 combats sauvages dans l'obscurite.",
   "rewardDesc": "2 000₽ + 5 Baies Ceriz"
  },
  "7": {
   "title": "Les Fantômes de la Tour de Lavanville",
   "desc": "Rejoignez Lavanville et bravez les esprits de la Tour Pokémon. Remportez 5 combats pour obtenir 3 Super Bonbons.",
   "rewardDesc": "4 500₽ + 3 Super Bonbons"
  },
  "48": {
   "title": "La Route 7 — Céladopole",
   "desc": "La Route 7 relie Safrania a Céladopole. Remportez 6 combats sauvages pour vous preparer a l'arene.",
   "rewardDesc": "2 500₽ + 1 Pierre Plante"
  },
  "8": {
   "title": "La Grâce Naturelle de Céladopole",
   "desc": "Rendez-vous dans la metropole de Céladopole et triomphez d'Erika pour remporter le Badge Prisme.",
   "rewardDesc": "5 000₽ + Pierre Plante + Orbe Vie"
  },
  "9": {
   "title": "Le Maître de l'Ombre à Parmanie",
   "desc": "Atteignez Parmanie et déjouez les pieges toxiques de Koga pour capturer le Badge Ame.",
   "rewardDesc": "6 000₽ + Restes"
  },
  "10": {
   "title": "Les Pouvoirs Psychiques de Safrania",
   "desc": "Pénétrez dans l'arene psychique de Safrania et vainquez Morgane (Sabrina) pour decrocher le Badge Marais.",
   "rewardDesc": "7 000₽ + Veste Combat"
  },
  "49": {
   "title": "Le Grand Souterrain",
   "desc": "Ouvrez l'onglet ⛏️ Mine et utilisez vos outils pour extraire et vendre 3 tresors deterres dans les roches de Kanto.",
   "rewardDesc": "4 000₽ + 2 Poussières Étoile"
  },
  "50": {
   "title": "Les Îles Écume — Route 19 et 20",
   "desc": "Partez vers les Chenaux 19 et 20 au sud de Parmanie. Remportez 8 combats sauvages en mer.",
   "rewardDesc": "4 000₽ + 10 Baies Oran"
  },
  "11": {
   "title": "Le Volcan Flamboyant de Cramois'île",
   "desc": "Traversez les chenaux maritimes jusqu'a Cramois'île et battez Auguste (Pyro) pour le Badge Volcan.",
   "rewardDesc": "8 000₽ + Pierre Feu & Pierre Eau"
  },
  "12": {
   "title": "L'Ultime Arène de Jadielle",
   "desc": "Retournez a Jadielle et affrontez le mystérieux Giovanni dans sa redoutable arene Sol pour obtenir le 8ᵉ Badge Kanto !",
   "rewardDesc": "10 000₽ + Mouchoir Choix + 5 Super Bonbons"
  },
  "51": {
   "title": "La Route 23 — L'Avant-Garde",
   "desc": "La Route 23 garde l'acces a la Route Victoire. Remportez 10 combats sauvages pour vous aguerri.",
   "rewardDesc": "5 000₽ + Mouchoir Choix"
  },
  "13": {
   "title": "Le Maître Suprême de Kanto",
   "desc": "Francchissez la Route Victoire et triomphez des 5 combats du Conseil 4 au Plateau Indigo pour devenir le nouveau Maître de Kanto !",
   "rewardDesc": "25 000₽ + Bandeau Choix + Lunettes Choix"
  },
  "14": {
   "title": "L'Oiseau de Glace des Îles Écume",
   "desc": "Explorez les profondeurs glacées des Îles Écume (Chenal 20) et triomphez de 10 Pokémon pour éveiller le légendaire Artikodin !",
   "rewardDesc": "Artikodin (Légendaire ✨)"
  },
  "15": {
   "title": "L'Oiseau de Foudre de la Centrale",
   "desc": "Pénétrez dans la Centrale Électrique abandonnée (Route 10) et vainquez 10 Pokémon pour faire apparaître Électhor !",
   "rewardDesc": "Électhor (Légendaire ✨)"
  },
  "16": {
   "title": "L'Oiseau de Feu de la Route Victoire",
   "desc": "Traversez les epreuves de la Route Victoire (Grotte) et remportez 10 combats pour defier Sulfura !",
   "rewardDesc": "Sulfura (Légendaire ✨)"
  },
  "17": {
   "title": "Les Archives du Manoir Pokémon",
   "desc": "Fouillez les ruines du Manoir Pokémon sur Cramois'île. Vainquez 10 Pokémon sauvages pour découvrir des secrets.",
   "rewardDesc": "15 000₽ + Charme Rapide + 10 Super Bonbons"
  },
  "18": {
   "title": "Le Clone Ultime de la Caverne Azurée",
   "desc": "Maintenant que vous êtes Maître de Kanto, accédez a la mystérieuse Caverne Azurée au nord d'Azuria. Triomphez de 15 Pokémon pour capturer Mewtwo !",
   "rewardDesc": "Mewtwo (Légendaire ✨) + 25 000₽"
  },
  "19": {
   "title": "Le Fabuleux Miraculeux du Parc Safari",
   "desc": "Explorez le vaste Parc Safari a Parmanie. Vainquez 15 Pokémon sauvages pour rencontrer et capturer le rarissime fabuleux Mew !",
   "rewardDesc": "Mew (Fabuleux ✨) + Charme Chroma + 50 000₽"
  },
  "31": {
   "title": "Le Salut du Prof. Orme",
   "desc": "À Bourg Geonif, parlez au Prof. Orme pour debutere officiellement votre aventure Johto.",
   "rewardDesc": "300₽ + 5 Baies Oran"
  },
  "20": {
   "title": "21. L'Appel de Johto — Bourg Geonif",
   "desc": "Bienvenue en Johto ! Parlez au Prof. Orme à Bourg Geonif puis remportez 3 combats sauvages pour débuter.",
   "rewardDesc": "500₽ + 5 Baies Oran"
  },
  "21": {
   "title": "22. L'Épreuve d'Argenta (Johto)",
   "desc": "Rendez-vous à Argenta et défiez Albert (Falkner) dans son arène Vol pour le Badge Zéphyr !",
   "rewardDesc": "1 000₽ + 5 Baies Sitrus"
  },
  "22": {
   "title": "23. La Forêt d'Écorcia",
   "desc": "À Écorcia, triomphez d'Hector (Bugsy) pour le Badge Essaim !",
   "rewardDesc": "1 500₽ + Bandeau Muscle"
  },
  "23": {
   "title": "24. La Plaine de Doublonville",
   "desc": "À Doublonville, battez Blanche (Whitney) pour le Badge Plaine !",
   "rewardDesc": "2 000₽ + Sable Doux"
  },
  "24": {
   "title": "25. La Brume d'Ébénelle",
   "desc": "À Rosalia, vainquez Mortimer (Morty) pour le Badge Brume !",
   "rewardDesc": "2 500₽ + Mouchoir Choix"
  },
  "25": {
   "title": "26. Le Choc d'Irisia",
   "desc": "Sur l'Île d'Irisia, battez Chuck pour le Badge Choc !",
   "rewardDesc": "3 000₽ + Joyau Pouvoir"
  },
  "26": {
   "title": "27. Le Minéral d'Oliville",
   "desc": "À Oliville, triomphez de Jasmine pour le Badge Minéral !",
   "rewardDesc": "3 500₽ + Restes"
  },
  "27": {
   "title": "28. Le Glacier d'Acajou",
   "desc": "À Acajou, vainquez Frédo (Pryce) pour le Badge Glacier !",
   "rewardDesc": "4 000₽ + Lunettes Choix"
  },
  "28": {
   "title": "29. Le Lever d'Ébénelle",
   "desc": "À Ébénelle, affrontez Sandra (Clair) pour le Badge Lever et devenir Maître de Johto !",
   "rewardDesc": "8 000₽ + Bandeau Choix + 5 Super Bonbons"
  },
  "60": {
   "title": "La Tour Brûlante : le Mystère des Bêtes",
   "desc": "À Rosalia, explorez la Tour Brûlante et affrontez 8 Pokémon pour libérer les trois Bêtes Légendaires : Raikou, Entei et Suicune !",
   "rewardDesc": "3 000₽ + 5 Super Bonbons"
  },
  "61": {
   "title": "Raikou, la Foudre Pourpre",
   "desc": "Sillonnez la Route 44 et remportez 10 combats pour faire surgir Raikou, la Bête Foudre !",
   "rewardDesc": "Raikou (Légendaire ✨)"
  },
  "62": {
   "title": "Entei, le Volcan Vivant",
   "desc": "Parcourez la Route 28 et remportez 10 combats pour réveiller Entei, la Bête Feu !",
   "rewardDesc": "Entei (Légendaire ✨)"
  },
  "63": {
   "title": "Suicune, l'Aurore Boréale",
   "desc": "Parcourez la Route 42 et remportez 10 combats pour croiser Suicune, la Bête Eau !",
   "rewardDesc": "Suicune (Légendaire ✨)"
  },
  "64": {
   "title": "Les Îles Whirl : Lugia, Gardien des Mers",
   "desc": "Plongez aux Îles Whirl et remportez 10 combats pour éveiller Lugia, le Pokémon de la Mer !",
   "rewardDesc": "Lugia (Légendaire ✨)"
  },
  "65": {
   "title": "La Tour Cendrée : Ho-Oh, Oiseau de Feu",
   "desc": "Gravissez la Tour Cendrée à Rosalia et remportez 10 combats pour faire apparaître Ho-Oh, l'Oiseau de Feu !",
   "rewardDesc": "Ho-Oh (Légendaire ✨)"
  },
  "66": {
   "title": "Le Bois aux Chênes : Celebi, Pokémon Fabuleux",
   "desc": "Dans le Bois aux Chênes, remportez 12 combats pour rencontrer Celebi, le Pokémon du Temps !",
   "rewardDesc": "Celebi (Fabuleux ✨)"
  }
 },
 "side": {
  "s1": {
   "title": "Le Collectionneur de Baies",
   "desc": "Le Jeune Régis veut voir vos talents : vainquez 10 Pokémon sauvages sur la Route 1.",
   "rewardDesc": "800₽ + 5 Baies Sitrus"
  },
  "s2": {
   "title": "Apprenti Dresseur",
   "desc": "Le Dresseur Red vous met au défi : remportez 15 combats sur la Route 2.",
   "rewardDesc": "1 200₽ + 5 Baies Prine"
  },
  "s3": {
   "title": "Chercheur de Pierres",
   "desc": "Le Pokémaniac Pierre vous envoie au Mont Sélénite : vainquez 12 Pokémon pour une Pierre Lune.",
   "rewardDesc": "1 500₽ + 1 Pierre Lune"
  },
  "s4": {
   "title": "Marin de Carmin",
   "desc": "Le Marin Brie veut des trésors : vendez 5 objets de la mine.",
   "rewardDesc": "1 500₽ + 1 Pépite"
  },
  "s5": {
   "title": "Gardiens de la Tour",
   "desc": "La Channeler Lila vous demande : vainquez 12 Pokémon à la Tour Pokémon.",
   "rewardDesc": "2 000₽ + 3 Super Bonbons"
  },
  "s6": {
   "title": "Vendeur de Céladopole",
   "desc": "Le Vendeur Marco veut agrandir sa collection : capturez 8 Pokémon différents.",
   "rewardDesc": "2 500₽ + 1 Pierre Plante"
  },
  "s7": {
   "title": "Dresseur de Parmanie",
   "desc": "La Dresseuse Nina vous défie sur la Route 19 : 12 victoires !",
   "rewardDesc": "2 000₽ + Restes"
  },
  "s8": {
   "title": "Scientifique de Cramois'île",
   "desc": "La Scientifique Aria explore le Manoir : vainquez 12 Pokémon pour une Pierre Feu.",
   "rewardDesc": "2 500₽ + 1 Pierre Feu"
  },
  "s9": {
   "title": "Gardien d'Oliville",
   "desc": "Le Gardien Olaf surveille la Route 38/39 : 10 victoires !",
   "rewardDesc": "2 000₽ + 10 Baies Oran"
  },
  "s10": {
   "title": "Maître Draco d'Ébénelle",
   "desc": "Le Maître Draco vous met à l'épreuve sur la Route 45 : 10 victoires !",
   "rewardDesc": "2 500₽ + 2 Super Bonbons"
  },
  "s11": {
   "title": "Gardien de Jadielle",
   "desc": "Le Gardien Jo défie les newcomers : remportez 12 combats sauvages sur la Route 22 !",
   "rewardDesc": "1 800₽ + 5 Baies Ceriz"
  },
  "s12": {
   "title": "Chercheuse d'Azuria",
   "desc": "La Chercheuse Lys traque les Pokémon rares : remportez 12 combats sur la Route 9 !",
   "rewardDesc": "2 200₽ + 5 Baies Sitrus"
  },
  "s13": {
   "title": "Collectionneuse de Céladopole",
   "desc": "La Collectionneuse Mia veut compléter son Pokédex : capturez 10 Pokémon différents !",
   "rewardDesc": "2 600₽ + Pierre Plante + Orbe Vie"
  },
  "s14": {
   "title": "Dresseur de Carmin",
   "desc": "Le Dresseur Ken surveille la Route 11 : 12 victoires sauvages !",
   "rewardDesc": "2 400₽ + 1 Pierre Foudre"
  },
  "s15": {
   "title": "Gardien de Lavanville",
   "desc": "Le Gardien Tom arpente la Route 12 : remportez 12 combats sauvages !",
   "rewardDesc": "2 400₽ + 4 Super Bonbons"
  },
  "s30": {
   "title": "La Team Rocket à Acajou",
   "desc": "Le Scientifique Pierre-Jean traque la Team Rocket : remportez 10 combats à Acajou !",
   "rewardDesc": "2 500₽ + Upgrade"
  },
  "s31": {
   "title": "Le Gyarados Rouge",
   "desc": "Le Pêcheur Requin traque le Lac Colère : 8 victoires !",
   "rewardDesc": "2 000₽ + Crocdure"
  },
  "s32": {
   "title": "La Tour Radio",
   "desc": "La Reporter Nina couvre Doublonville : 10 victoires !",
   "rewardDesc": "2 200₽ + 2 Super Bonbons"
  },
  "s33": {
   "title": "Les Évolutions des Caves Jumelles",
   "desc": "Le Spécialiste Évoli explore les Caves Jumelles : 10 victoires !",
   "rewardDesc": "2 000₽ + Rocher Royal"
  },
  "s34": {
   "title": "Le Dresseur de la Route 30",
   "desc": "Le Jeune Hugo défie la Route 30 : 12 victoires !",
   "rewardDesc": "1 800₽ + 5 Baies Sitrus"
  },
  "s35": {
   "title": "Le Garde-Fort d'Écorcia",
   "desc": "Le Garde Boris surveille Écorcia : 10 victoires !",
   "rewardDesc": "2 000₽ + Etiquette Maléfique"
  },
  "s36": {
   "title": "Collectionneuse de Doublonville",
   "desc": "La Collectionneuse Églantine veut 8 espèces : capturez-les !",
   "rewardDesc": "2 600₽ + Orbe Vie"
  },
  "s37": {
   "title": "Le Spécialiste de la Route 35",
   "desc": "Le Dresseur Marius arpente la Route 35 : 12 victoires !",
   "rewardDesc": "2 200₽ + Graine Miracle"
  },
  "s38": {
   "title": "Le Marin d'Oliville",
   "desc": "Le Marin Joachim surveille Oliville : 10 victoires !",
   "rewardDesc": "2 000₽ + Eau Mystique"
  },
  "s39": {
   "title": "Le Maître de la Tour Chétiflor",
   "desc": "Le Moine Bao teste les initiés : 8 victoires à la Tour Chétiflor !",
   "rewardDesc": "1 500₽ + Cuillère Tordue"
  },
  "s40": {
   "title": "Le Mythe des Ruines d'Alpha",
   "desc": "L'Archéologue Solène décrypte les Ruines d'Alpha : 8 victoires !",
   "rewardDesc": "1 800₽ + 3 Super Bonbons"
  }
 },
 "repeatable": {
  "r1": {
   "title": "Chasseur de Trésors",
   "desc": "Vendez 5 trésors déterrés à la mine.",
   "rewardDesc": "1 000₽"
  },
  "r2": {
   "title": "Dresseur Itinérant",
   "desc": "Remportez 25 combats sauvages (n'importe où).",
   "rewardDesc": "5 Baies Prine"
  },
  "r3": {
   "title": "Collectionneur",
   "desc": "Capturez 10 Pokémon (n'importe où).",
   "rewardDesc": "1 500₽"
  },
  "r4": {
   "title": "Mineur Acharné",
   "desc": "Vendez 10 trésors à la mine.",
   "rewardDesc": "1 Super Bonbon"
  },
  "r5": {
   "title": "Explorateur Aguerri",
   "desc": "Remportez 50 combats sauvages (n'importe où).",
   "rewardDesc": "1 Mouchoir Choix"
  },
  "r6": {
   "title": "Vendeur d'Objets",
   "desc": "Vendez 8 trésors à la mine.",
   "rewardDesc": "2 000₽"
  },
  "r30": {
   "title": "Patrouille Route 29",
   "desc": "Remportez 15 combats sauvages sur la Route 29.",
   "rewardDesc": "5 Baies Oran"
  },
  "r31": {
   "title": "Patrouille Route 34",
   "desc": "Remportez 20 combats sauvages sur la Route 34.",
   "rewardDesc": "1 Eau Mystique"
  },
  "r32": {
   "title": "Patrouille Route 45",
   "desc": "Remportez 20 combats sauvages sur la Route 45.",
   "rewardDesc": "1 Crocdure"
  },
  "r33": {
   "title": "Chasseur de Baies Johto",
   "desc": "Capturez 10 Pokémon en Johto.",
   "rewardDesc": "1 500₽"
  },
  "r34": {
   "title": "Grotte Obscure",
   "desc": "Remportez 12 combats dans la Grotte Obscure.",
   "rewardDesc": "1 Rocher Royal"
  },
  "r35": {
   "title": "Bois aux Chênes",
   "desc": "Remportez 15 combats dans le Bois aux Chênes.",
   "rewardDesc": "1 Graine Miracle"
  },
  "r36": {
   "title": "Pêcheur du Chenal 40",
   "desc": "Remportez 15 combats sur le Chenal 40.",
   "rewardDesc": "1 Eau Mystique"
  },
  "r37": {
   "title": "Collecteur Johto",
   "desc": "Capturez 12 Pokémon différents.",
   "rewardDesc": "2 000₽ + 2 Super Bonbons"
  },
  "r38": {
   "title": "Mineur d'Acajou",
   "desc": "Vendez 6 trésors à la mine.",
   "rewardDesc": "2 000₽"
  }
 }
};

