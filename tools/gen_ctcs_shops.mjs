#!/usr/bin/env node
/**
 * gen_ctcs_shops.mjs — Generates src/data/ctcs-shop-data.js
 *
 * Phase 26: every TM/HM of the proper game versions is purchasable, spread
 * across ALL shops of both regions to force exploration.
 *
 * Canonical rule:
 *  - KANTO shops (FRLG)  → only TMs teaching 1st-generation moves.
 *  - JOHTO shops (HGSS)  → 2nd-generation move TMs + 1st-gen leftovers.
 *  - 3rd-generation moves and later → never sold (version honesty).
 *
 * Each TM is sold in EXACTLY ONE shop (maximal exploration), with a
 * thematic preference (same-type gym city) then balanced distribution.
 * Missing prices are computed in tiers by power.
 *
 * Usage: node tools/gen_ctcs_shops.mjs   (idempotent, fails loudly on anomaly)
 */
...
// ── 1st-generation moves (RBY/FRLG — canon) ────────────────────────
...
// ── 2nd-generation moves (HGSS — canon) ──────────────────────────────
...
// ── Shop themes (types normalized to lowercase English) ────
...
if (!ITEMS || !MOVES) throw new Error('ITEMS or MOVES not found');

// ── Collect all TMs (ct* keys, with moveId) ────────────────────────
const ctItems = [];
...
if (ctItems.length < 100) throw new Error(`not enough TMs detected: ${ctItems.length}`);

// ── Classification by generation ────────────────────────────────────────
const rows = ctItems.map(({ key, moveId, price }) => {
  const mv = MOVES[moveId];
  if (!mv) throw new Error(`${key} → unknown moveId in MOVES: ${moveId}`);
...
// ── Distribution: thematic preference then balancing ─────────────────
const assign = {}; // shopId -> [rows]
const counts = {};
const inited = [...KANTO_SHOPS, ...JOHTO_SHOPS];
inited.forEach((s) => { assign[s] = []; counts[s] = 0; });
const themed = new Set(); // keys already placed by theme
for (const region of ['kanto', 'johto']) {
  ...
  // Power combos (≥90) at Indigo/Blackthorn via dragon/endgame theme priority
  for (const r of pool) {
  ...
  // Remainder: balanced distribution
...
if (once.size !== soldKeys.length) throw new Error('a TM is sold in several shops');
if (soldKeys.length !== sellable.length) throw new Error(`uncovered sellable TMs: ${sellable.length - soldKeys.length}`);
for (const k of unsold) if (once.has(k)) throw new Error(`gen 3+ TM on sale: ${k}`);
for (const s of KANTO_SHOPS) for (const r of assign[s]) if (r.gen !== 1) throw new Error(`shop ${s} (Kanto) sells a non-gen1 TM: ${r.key}`);

// ── Sort by ascending price for shop display ─────────────────────
...
const file = `// GENERATED by tools/gen_ctcs_shops.mjs — do not edit by hand.
// Passe 26: TM/HM shop stock (canonical: Kanto = gen 1 moves,
// Johto = gen 2 + gen 1 leftovers; each TM sold in exactly ONE shop).
const CTCS_SHOP_STOCK = ${JSON.stringify(stock, null, 1)};
// Prices computed for TMs that had none (tiers by power).
const CTCS_PRICES = ${JSON.stringify(prices, null, 1)};
// Meta (move taught + generation) — used by tests and the display.
const CTCS_META = ${JSON.stringify(meta, null, 1)};
// TMs deliberately NEVER sold (3rd-generation-and-later moves).
const CTCS_UNSOLD = ${JSON.stringify(unsold, null, 1)};
// Missing prices applied on load (after items-data.js).
(function(){
